...
class GameManager:
    ...
    def get_apppath(self):
        """
        Get path to current application, useful for Popen.
        """
        def remove_dup(seq):
            seen = set()
            seen_add = seen.add
        ...
        if sys.argv[0].startswith("./"):
            no_dot_slash = sys.argv[0].replace("./", "")
            apppath.append(join(os.getcwd(), no_dot_slash))
        else:
            apppath = sys.argv
        if sys.executable:
            apppath = [sys.executable] + apppath
        apppath = remove_dup(apppath)
        return apppath
    ...
    def run_game(self, game_name):
        ...
        self.cleanup()
        # Start game.
        path = self.get_apppath()
        path.extend(["-r", game_name])
        process = Popen(path)
